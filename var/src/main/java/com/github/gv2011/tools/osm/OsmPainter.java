...
public class OsmPainter implements AutoCloseableNt{

  private static final QName V = QName.valueOf("v");
  private static final QName K = QName.valueOf("k");
  private static final QName LAT = QName.valueOf("lat");
  private static final QName LON = QName.valueOf("lon");
  private static final QName ID = QName.valueOf("id");

  public static void main(final String[] args) throws Exception {
//    try(OsmPainter osmAnalyzer = new OsmPainter()){
//      osmAnalyzer.analyze(
...
//  }
  ...
  private void handleElement(final XMLEventReader eventReader, final Optional<Element> parent) {
    ...
    else if(name.equals("node")) e = handleNode(parent, element);
  ...
  private Element handleNode(final Optional<Element> parent, final StartElement node) {
    final Optional<Long> id = Optional.ofNullable(node.getAttributeByName(ID))
      .map(Attribute::getValue)
      .map(Long::parseLong)
    ;
    final Optional<Double> lat = Optional.ofNullable(node.getAttributeByName(LAT))
      .map(Attribute::getValue)
      .map(Double::parseDouble)
    ;
    final Optional<Double> lon = Optional.ofNullable(node.getAttributeByName(LON))
      .map(Attribute::getValue)
      .map(Double::parseDouble)
    ;
